...
class public_img(): 
    ...
    def img_box_lk(self,img2,max_arce,min_arce):
        ...
        imgss = img.copy()
        imgss2 = img.copy()
        imgss2 = cv2.cvtColor(imgss2, cv2.COLOR_GRAY2RGB)
        imgss = cv2.cvtColor(imgss, cv2.COLOR_GRAY2RGB)
        cv2.imshow("DB",img)
        contours, _ = cv2.findContours(img,cv2.RETR_TREE , cv2.CHAIN_APPROX_SIMPLE)  
        for contour in contours:  
            x, y, w, h = cv2.boundingRect(contour) 
            cv2.rectangle(imgss2, (x, y), (x+w , y+h), (255, 0, 255), 1)
            cv2.imshow("xiess2",imgss2)
            arce = w*h 
            if max_arce> arce >= min_arce:
                ...
                cv2.rectangle(imgss, (x_min, y_min), (x_max , y_max), (0, 0, 255), 1)
                cv2.rectangle(img2, (x_min, y_min), (x_max , y_max), (0, 255, 0), 1)
    ...
    def eroded_and_dilated_img(self,img):
        ...
        img_blur=cv2.GaussianBlur(gray,(5,5),5)
        img_thresh_blur=cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,25,3)
        ...
        # 腐蚀 ---> 白色前 ，黑色背景
        dilated = cv2.dilate(img_thresh_blur, kernel)
...
if __name__ == '__main__':  
    ...
    output_dir = ".//zi//"
    image_1 = ".//testimg//testa//c_img//image11.jpg" 
