...
def predict(image_path, model, deskew_func, hog_func, pca, max_len):
    """推理"""
...
if __name__ == "__main__":
    # 模型路径
    letters_digits_model_path = "../model/svm_letters_digits.dat"
    # 加载模型和最大HOG描述符长度以及PCA模型
    letters_digits_model, max_len, pca = load_model(letters_digits_model_path)
    # 图像路径
    img_path_letters_digits = 'D:\\code\\pythonProject\\test_image.png'
    img = cv2.imread(img_path_letters_digits)
    # 预测
    result_letters_digits = predict(img_path_letters_digits, letters_digits_model, deskew, preprocess_hog, pca, max_len)
